...
func NewTableServiceProcessor(handler TableService) *TableServiceProcessor {
	self63 := &TableServiceProcessor{common.NewBaseServiceProcessor(handler)}
	self63.AddToProcessorMap("get", &tableServiceProcessorGet{handler: handler})
	self63.AddToProcessorMap("put", &tableServiceProcessorPut{handler: handler})
	self63.AddToProcessorMap("increment", &tableServiceProcessorIncrement{handler: handler})
	self63.AddToProcessorMap("remove", &tableServiceProcessorRemove{handler: handler})
	self63.AddToProcessorMap("scan", &tableServiceProcessorScan{handler: handler})
	self63.AddToProcessorMap("batch", &tableServiceProcessorBatch{handler: handler})
	return self63
}
