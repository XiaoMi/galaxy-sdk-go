...
func NewEMQBaseServiceClientFactory(t thrift.TTransport, f thrift.TProtocolFactory) *EMQBaseServiceClient {
	return &EMQBaseServiceClient{Transport: t,
		ProtocolFactory: f,
...
func (p *EMQBaseServiceClient) sendGetServiceVersion() (err error) {
...
func (p *EMQBaseServiceClient) recvGetServiceVersion() (value *Version, err error) {
...
func (p *EMQBaseServiceClient) sendValidClientVersion(clientVersion *Version) (err error) {
	oprot := p.OutputProtocol
	if oprot == nil {
		oprot = p.ProtocolFactory.GetProtocol(p.Transport)
		p.OutputProtocol = oprot
...
func (p *EMQBaseServiceClient) recvValidClientVersion() (err error) {
	iprot := p.InputProtocol
	if iprot == nil {
		iprot = p.ProtocolFactory.GetProtocol(p.Transport)
		p.InputProtocol = iprot
...
func NewEMQBaseServiceProcessor(handler EMQBaseService) *EMQBaseServiceProcessor {
	...
	self4 := &EMQBaseServiceProcessor{handler: handler, processorMap: make(map[string]thrift.TProcessorFunction)}
	self4.processorMap["getServiceVersion"] = &eMQBaseServiceProcessorGetServiceVersion{handler: handler}
	self4.processorMap["validClientVersion"] = &eMQBaseServiceProcessorValidClientVersion{handler: handler}
	return self4
}
...
type eMQBaseServiceProcessorGetServiceVersion struct {
	handler EMQBaseService
}

func (p *eMQBaseServiceProcessorGetServiceVersion) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	args := GetServiceVersionArgs{}
	...
	if retval, err2 = p.handler.GetServiceVersion(); err2 != nil {
		switch v := err2.(type) {
		case *GalaxyEmqServiceException:
			result.E = v
		default:
...
type eMQBaseServiceProcessorValidClientVersion struct {
	handler EMQBaseService
}

func (p *eMQBaseServiceProcessorValidClientVersion) Process(seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	args := ValidClientVersionArgs{}
	if err = args.Read(iprot); err != nil {
		iprot.ReadMessageEnd()
		x := thrift.NewTApplicationException(thrift.PROTOCOL_ERROR, err.Error())
